...
function App(){
 ...
 return <div className="shell">
  ...
  <div className="layout">
   ...
   {tab==="overview"&&<><div className="stats">{[["Current UTCI",selected.utci+"°","Live reading for "+selected.name,"orange"],["Mortality Risk Index",selected.mri,selected.risk+" · adjusted risk","red"],["High-risk cities",zones.filter(z=>z.risk==="Orange"||z.risk==="Red").length,"Require immediate review","blue"],["Healthcare capacity",selected.healthcare+"%","Available beds · "+selected.name,"green"]].map(x=><div className={"stat "+x[3]} key={x[0]}><small>{x[0]}</small><strong>{x[1]}</strong><span>{x[2]}</span></div>)}</div><div className="grid"><div className="wide"><MapPanel/></div><div className="card panel"><div className="panel-head"><div><h3>City risk summary</h3><p>Ranked by adjusted heat-health risk</p></div><span className="muted">{zones.length} cities</span></div><input className="search" placeholder="Search city..." value={query} onChange={e=>setQuery(e.target.value)}/><div className="zone-list">{filtered.map(z=><button className={"zone-row "+(selected.id===z.id?"selected":"")} onClick={()=>setSelected(z)} key={z.id}><span><b>{z.name}</b><small>UTCI {z.utci}°C · MRI {z.mri}</small></span><Badge risk={z.risk}>{z.risk}</Badge></button>)}</div></div></div><div className="grid lower"><Details/><ForecastPanel/></div></>}
